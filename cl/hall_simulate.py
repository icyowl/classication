...
@njit(cache=True)
def simulate(setting: int, p: np.ndarray, outs: np.ndarray, safs: np.ndarray, seed=0) -> np.ndarray:
    '''
    ...
    saf = sum([safs[x] for x in result])
    game = idx
    bb = (result < 3).sum()
...
@njit(cache=True)
def _simulate(setting: int, seed=0) -> np.ndarray:
    '''
    入力された設定値と、乱数シード値から遊技機のシュミレーション値を返す
    条件1: 2022年2月実績のアウト(target_outs)まで回す
    条件2: 実績のアウトとその一番近い数値の差が、8枚以上の場合、...
...
def loop(setting: int, size: int, step=10):
    ...
    rows = []
    for i in range(size):
        arr = simulate(setting, p, out, saf, seed=i*step)
        rows.append(arr)
    return rows
